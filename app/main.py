...
def get_settlement(group_id: str, db: Session = Depends(get_db)):
    ...
    for expense in group.expenses:
        ...
        # naive approach: every split member (other than payer) generates
        # one individual debt record against the payer for this expense
        naive_count += sum(1 for s in expense.splits if s.member_id != expense.paid_by_member_id)
